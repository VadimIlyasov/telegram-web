...
//                     if (typeof list['users'][id] !== 'undefined') {
//                         contactData = list['users'][id];
//                         contactsHTML += contactTpl({id: id, type: 'user', name: contactData.first_name, lastMessage: message, time: date.getUTCHours().pad() + ':' + date.getMinutes().pad(), counter: el.unread_count});
//                     }
//                 } else {
//                     if (typeof list['chats'][id] !== 'undefined') {
//                         contactData = list['chats'][id];
//                         contactsHTML += contactTpl({id: id, type: 'chat', name: contactData.title, lastMessage: message, time: date.getUTCHours().pad() + ':' + date.getMinutes().pad(), counter: el.unread_count});
//                     }
// >>>>>>> 046607360107199ddf94e0d27fc94b08262ee1af
//                 }
//             });
//
//             $contactsList.append(contactsHTML);
...
//                 let date = new Date(message.date * 1000);
//
//                 $('.messages-list').prepend(messageTpl({id: id, message_type: (message.from_id == 517665176)?'my-message':'', message: message.message, from: message.from_id, time: date.getUTCHours().pad() + ':' + date.getMinutes().pad()}));
//
//                 /**
//                  * message.from_id - Sender ID
//                  * message.date - Date
